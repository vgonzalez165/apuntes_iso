...
El Administrador del servidor es la única herramienta gráfica que permite trabajar con grupos de almacenamiento y crear discos virtuales. También permite realizar algunas (pero no todas) las operaciones estándar sobre discos físicos.
...
- En el apartado Grupos de almacenamiento por ahora solo está el Grupo primordial, que contiene todos los discos físicos que no están en ningún grupo de almacenamiento y pueden añadirse a uno.
- Si hacemos click con el botón derecho podremos escoger Nuevo grupo de almacenamiento.
 
- Esto lanzará el asistente para la creación de grupos de almacenamiento. Nos pedirá los siguientes datos:
...
- En la parte inferior izquierda, en el apartado Discos virtuales, hacemos click en la parte superior en Tareas y seleccionamos Nuevo disco virtual. Esto lanzará el asistente para la creación de un disco virtual. Algunos aspectos a tener en cuenta en este asistente son:
    ...
    - En Aprovisionamiento se indicará cómo se va a reservar el espacio del disco virtual dentro del grupo de almacenamiento. Si escogemos aprovisionamiento delgado solo se tomará del grupo de almacenamiento el espacio realmente ocupado por datos en el disco virtual. Este modo permite discos virtuales más grandes que el espacio disponible en los discos físicos. El aprovisionamiento fijo reserva todo el espacio asignado al disco virtual.
...
Desde ambas herramientas es posible crear particiones y no hay diferencia entre ellas más allá de las diferencias en la interfaz ya que, aunque lanzan asistentes diferentes, las opciones dentro de ellos son las mismas.
...
El primer cmdlet que tenemos que conocer es `Get-PhysicalDisk`, que no muestra información sobre los discos físicos que hay instalados en el sistema. Entre la información mostrada hay una columna denominada CanPool que indicará si ese disco se puede añadir a un grupo de almacenamiento (True) o no (False).
...
- `-FriendlyName`: nombre con que se identificará el grupo de almacenamiento.
...
Aquí el problema es que no es suficiente con indicar el nombre del disco en el último parámetro, sino que hay que pasar el objeto disco físico completo. Para ello utilizaremos los paréntesis, que permiten ejecutar un comando para pasar su salida a otro. El modo de usarlo se puede ver en el siguiente ejemplo.

```powershell
PS C:\>New-StoragePool -FriendlyName "Datos” -StorageSubSystemFriendlyName “Storage Spaces” -PhysicalDisks (Get-PhysicalDisk PhysicalDisk2)
```

Otra posibilidad es almacenar el objeto disco en una variable. Las variables en PowerShell se identifican por un nombre precedido del símbolo dólar (`$`).

El primer paso es asignar un valor a las variables utilizando el operador igual (=). Las variables en PowerShell pueden contener cualquier tipo de valor. En el siguiente ejemplo la variable va a almacenar una cadena de texto. 
 
Como se puede ver en el ejemplo, para invocar la variable simplemente hay que ponerla y será reemplazada automáticamente por su valor.
Si queremos que la variable contenga la salida de un comando hay que utilizar los paréntesis de la siguiente forma:
 
Teniendo esto en cuenta, ahora podemos hacer más sencillo el comando para crear los grupos de almacenamiento haciendo lo siguiente:
...
- `-AccessData`: letra que le vamos a asignar al volumen.
